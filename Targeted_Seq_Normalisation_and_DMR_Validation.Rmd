...
clinical_markers<-readxl::read_excel("/home/arathge/from-rafael/WGBS_ShortTableCorrelations.xlsx")
...
p7 <- all_data %>% dplyr::group_by(seq_method) %>% 
  ...
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5, face="bold", size=16), axis.text.x = element_text(angle = 45, vjust = 0.8, hjust = 1), legend.position = "bottom")

# ggsave(
# "/home/arathge/output/plots/ACSS_cardiac_ccfDNA/Boxplot_raw_data_all_common_tiles.tiff",
# device = "tiff",
# plot = p7)
...
p32 <- autoplot(prcomp(overlap_log_pca %>% dplyr::select(starts_with("chr"))), data = overlap_log_pca, colour = "seq_method") +
  labs( colour="Method") +
  geom_text(size=2,aes(label= sample_ids, colour=seq_method), check_overlap = TRUE, nudge_y = 0.02) +
  scale_colour_manual(values = c("darkorchid4", "darkturquoise"))+
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5, face="bold", size=20), aspect.ratio = 1, legend.position="none")


# ggsave(
# "/home/arathge/output/plots/ACSS_cardiac_ccfDNA/PCA_raw_data_all_common_tiles.tiff",
# device = "tiff",
# plot = p32)
...
# ggsave(
# "/home/arathge/output/plots/ACSS_cardiac_ccfDNA/PCA_log_transformed_normalised_combat_all_common_tiles.tiff",
# device = "tiff",
# plot = p35)
...
p69 <- condition_DM_perc_meth_pca %>%dplyr::mutate(condition = if_else(condition == "Control", "Healthy", condition)) %>%
  ...
  #geom_text_repel(size=3,aes(label= sample_ids, colour=condition), force=3, max.overlaps = 20) +
  ...
  # theme(plot.title = element_text(hjust = 0.5, face="bold", size=20), aspect.ratio = 1, legend.position="right", axis.title.y = element_text(angle=90,vjust =2, face = "bold"), axis.title.x = element_text(vjust = -0.2, face="bold"), legend.title = element_text(face="bold")
# )
...
#         
# )+
  scale_color_manual(values=c("#919494", "khaki3 ", "tan4"))+
  facet_wrap(vars(condition))
p66
```


```{r Original_Code_Chunk_from_above_not_valid_anymore}
# compare_discovery_validation <- overlap_DM_log_normalised_combat_log_fold_change_no_log_mean_diff_qvalues %>% 
#   dplyr::rename(meth.diff.WGBS.validation = meth.diff.WGBS, meth.diff.targeted.validation=meth.diff.targeted, threshold_25_validation=threshold_25, threshold_25_qvalue_discovery=threshold_25_qvalue) %>% 
#   dplyr::left_join(dplyr::select(data_for_scatterplot,  seqnames, start, end, group, meth.diff.y), by = c("chrom"="seqnames", "start", "end", "condition"="group")) %>% 
#   dplyr::rename(meth.diff.WGBS.discovery=meth.diff.y) %>% 
#   dplyr::mutate(threshold_25_cohort_comparison = ifelse((meth.diff.WGBS.discovery >= 25 & meth.diff.targeted.validation >= 25 | meth.diff.WGBS.discovery <= -25 & meth.diff.targeted.validation <= -25), "DMR validated", "DMR not validated"))
# 
# p66 <- compare_discovery_validation %>% mutate(threshold_25_cohort_comparison = if_else(threshold_25_cohort_comparison == "DMR not validated", "not validated  ", "validated")) %>%
#   ggplot(aes(x = meth.diff.targeted.validation, y = meth.diff.WGBS.discovery, colour=threshold_25_cohort_comparison)) +
#   geom_point(shape = 16, alpha=0.6, size=1) +
#   geom_vline(xintercept = c(-25, 25), linetype="dotted")+
#   geom_hline(yintercept = c(-25, 25), linetype="dotted")+
#   ylab("\u0394 Methylation [%]\nWGBS Discovery Cohort") +
...
#   #labs(title = "Log-Backtransformed, Quantile Normalised and ComBat", 
#   labs(colour="DMR") +
#   stat_cor(label.x =(-55) , label.y=c(-5, 5), method="pearson", size=2.5)+
#   ylim(-lim, lim) +
#   xlim(-lim, lim) +
#   theme_Publication()+
#   theme(aspect.ratio = 1)+
...
# #         
# # )+
#   scale_color_manual(values=c("#919494", "tan4"))+
#   facet_wrap(vars(condition))
# p66
#annotate_figure(ggarrange(p66 , nrow=1, ncol=1), text_grob("DM Tiles", face = "bold", size=16))
```

```{r Number_of_Validated_Tiles_Validatioin_Cohort}
...
ggsave(bg ="white",
"/local/AAkalin_cardiac/Results/cardiac/Plots/Figure6.tiff",
device = "tiff",
plot = figure_6
)


figureS5 <- ggarrange(p7,
          ggarrange(p32, p35, ncol = 2, labels =c("B", "C")),
          nrow=2,
          labels="A")
ggsave(bg ="white",
"/local/AAkalin_cardiac/Results/cardiac/Plots/FigureS5.tiff",
device = "tiff",
plot = figureS5
)
```
