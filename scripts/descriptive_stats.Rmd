...
table_markers <- merge(suppl_tbl_2, table_markers, by = "Sample", all.x=TRUE)
...
Table_S1 <- table_markers %>%
  dplyr::select(!c("total_amount_cfDNA_ug", "total_amount_cfDNA_ug")) %>%
  dplyr::rename(
  ...
  dplyr::mutate(`Sequencing method`= c(
  rep("WGBS", 29
      ))) %>%
  dplyr::relocate(`Sequencing method`, .after = `Condition group`)
...
table_markers_ACS <- table_markers_ACS %>% dplyr::rename("ccfDNA [ng/ml]"= "ng ccfDNA /mL plasma",
                    "TroponinT [ng/l]" = "TroponinT (ng/l)",
                    "LVEF [%]" ="LVEF(%)",
                    "CK [U/l]" = "CK (U/l)",
                    "CK max [U/l]" = "CK max (U/l)",
...
corr_biomarkers <- cor(table_markers_ACS %>% dplyr::select(str_sort(c("ccfDNA [ng/ml]", "CK-MB max [U/l]", "CK max [U/l]",  "CRP [mg/dl]", "LVEF [%]", "TroponinT [ng/l]", "CK [U/l]", "CK-MB [U/l]"), decreasing=FALSE)), use = "complete.obs")
```
...
Figure1C <- (corr_biomarkers * upper.tri(corr_biomarkers, diag = TRUE)) %>%
  ...
  scale_size_continuous(labels = expression("">=10^{-1},"">=10^{-2},"">=10^{-3},"">=10^{-4}), breaks = c(1, 2, 3, 4)) + 
  labs(colour = "Pearson's r", size = "p-value") + 
  guides(
    fill = "none",
...
clinical_markers <- str_sort(c("ccfDNA [ng/ml]", "CK [U/l]", "CK max [U/l]", "CK-MB [U/l]", "CK-MB max [U/l]", "CRP [mg/dl]", "LVEF [%]", "TroponinT [ng/l]"), decreasing = FALSE)
...
misclassification_rates$Marker <- factor(misclassification_rates$Marker, levels =str_sort(c("ccfDNA [ng/ml]", "CK [U/l]", "CK max [U/l]", "CK-MB [U/l]", "CK-MB max [U/l]", "CRP [mg/dl]", "LVEF [%]", "TroponinT [ng/l]"), decreasing = TRUE))
...
ggplot2::ggsave(
  "/local/AAkalin_cardiac/Results/cardiac/Plots/Figure1_v2.pdf",
  plot = FigureABC, width = 210, units = "mm", height = 210
)
ggplot2::ggsave(
  bg = "white",
  "/local/AAkalin_cardiac/Results/cardiac/Plots/Figure1_v2.png",
  plot = FigureABC, width = 210, units = "mm", height = 210
)
FigureABC
```
...
#Upset plot could not be converted to ggplot object via ggplotify as bug in upset package, thus here workaround based on
#https://github.com/wilkelab/cowplot/issues/11#issuecomment-235643412
#https://github.com/hms-dbmi/UpSetR/issues/105#issuecomment-571066887
...
for (n in marker_names) {
  ...
  test <- lm(DMR_percMeth_ACS_markers[[n]] ~ DMR_percMeth_ACS_markers$`ccfDNA [ng/ml]`)
  ...
  for (i in dmr_names) {
    # tmp <- DMR_percMeth_ACS_markers %>% select( all_of(n), all_of(i),`ccfDNA [ng/ml]` )

    # LM biomarkers ~ methylation DMRs + cfDNA
    test2 <- lm(DMR_percMeth_ACS_markers[[n]] ~ DMR_percMeth_ACS_markers[[i]] + scale(DMR_percMeth_ACS_markers$`ccfDNA [ng/ml]`))
  ...
  test_all_adj <- test_all_adj[!grepl("ccfDNA [ng/ml]", row.names(test_all_adj)), ]
...
```{r}
stemi_for_merge_lm <- final_DMR_df_SDs_sorted_annotated$ACS_Stemi %>% 
  dplyr::select(seqnames, start, end, gene, distTSS, meth.diff, qvalue, present_in_groups, enhancer_roadmap, annot.id, specific, heart_disease_related_disgenet, heart_disease_name_disgenet)

nstemi_for_merge_lm <- final_DMR_df_SDs_sorted_annotated$ACS_Nstemi %>% 
  dplyr::select(seqnames, start, end, gene, distTSS, meth.diff, qvalue, present_in_groups, enhancer_roadmap, annot.id, specific, heart_disease_related_disgenet, heart_disease_name_disgenet)

UA_for_merge_lm <- final_DMR_df_SDs_sorted_annotated$ACS_iAP %>% 
  dplyr::select(seqnames, start, end, gene, distTSS, meth.diff, qvalue, present_in_groups, enhancer_roadmap, annot.id, specific, heart_disease_related_disgenet, heart_disease_name_disgenet)
...
narrow_DMR <- treat_LM %>% 
  tidyr::separate(DMR, c("chrom", "start", "end", "suffix"), sep="\\.") %>% 
  dplyr::filter(is.na(suffix)) %>%
  dplyr::select(-suffix) %>% 
  tidyr::unite(DMR, chrom, start, end, sep=".")

#saveRDS(narrow_DMR, "/local/rcuadrat/cfdna_wgbs/narrow_DMRs.RDS")
#write_tsv(narrow_DMRs %>% dplyr::select(DMR), file = "/local/AAkalin_cardiac/metadata/Supplementary_Table_4_Disease_distinguishing_DMRs.tsv")
...
# saveRDS(colnames(percMeth_ACS_DMRs_sig_lm),"/local/AAkalin_cardiac/Results/cardiac/RDS/top_DMR_treatment.RDS")
```
