...
clinical_markers <- c("TroponinT [ng/l]", "LVEF[%]", "CK [U/l]", "CK max [U/l]", "CK-MB [U/l]", "CK-MB max [U/l]", "ccfDNA [ng/mL]")
misclassification_rates <-c()

for (marker in clinical_markers){
    obj<- tibble::as_tibble(table_markers_ACS %>% 
                              dplyr::left_join(suppl_tbl_2 %>% 
                                                 dplyr::select(Sample, cfDNA_ug_per_ml_plasma))) %>%
      dplyr::rename("ccfDNA [ng/mL]"="cfDNA_ug_per_ml_plasma",
                    "TroponinT [ng/l]" = "TroponinT (ng/l)",
                    "LVEF[%]" ="LVEF(%)",
                    "CK [U/l]" = "CK (U/l)",
                    "CK max [U/l]" = "CK max (U/l)",
    ...
    classification_table<-tibble(True_ACS_type= obj %>% filter(!is.na(.[[2]])) %>% select(treatment) %>% pull, predicted_ACS_type=predicted_ACS_type$prediction)
...
Figure1B <- ggplot(arrange(misclassification_rates, Marker), aes(x = Marker, y = value)) +
  geom_bar(stat = "identity") +
  labs(y="Misclassification \nrate")+
  theme_Publication() +
  theme(
...
png(file = "/local/AAkalin_cardiac/Results/cardiac/Plots/FigureS2.png",height = 8,width = 20)
