...
#Export significant methylation percentages for Supplementary Table 5
##########
methylDiff_WGBS_exp <- methylDiff_WGBS %>%
  map2(names(.), ~ as_tibble(.x) %>% 
                   tidyr::unite(tiles, seqnames, start, end, sep=".") %>% 
                   add_column(condition=.y)) %>% 
  bind_rows()

meth_perc_all_tiles_WGBS_exp <- meth_perc_all_tiles_WGBS %>% 
  dplyr::filter(rownames(.) %in% methylDiff_WGBS_exp$tiles)

write_tsv(meth_perc_all_tiles_WGBS_exp, file = "/local/AAkalin_cardiac/metadata/Supplementary_Table_5_Methylation_Percentages_Significant_DMRs.tsv")
##########
...
saveRDS(overlap_log_normalised_combat_no_log, "/local/AAkalin_cardiac/Results/cardiac/RDS/overlap_log_normalised_combat_no_log.RDS")
```
...
saveRDS(condition_DM_perc_meth_pca, "/local/AAkalin_cardiac/Results/cardiac/RDS/condition_DM_perc_meth_pca.RDS")
...
```{r PCA_log_transformed_and_comBat, error=FALSE, message=FALSE, warning=FALSE}
