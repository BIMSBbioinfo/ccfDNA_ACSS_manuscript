...
Figure1B<-corrplot(corr_biomarkers, tl.col = "black", tl.srt = 30, bg = "White",
         type = "lower")
# dev.off()
```
...
ggplot(all_annot_summary_alltypes, aes(x=annot.type, y=n,fill=group))+
  ...
  scale_fill_manual(values=group.colors_3) + 
  scale_fill_Publication() +
  theme_Publication()
...
test<-ggplot(pca_out%>% dplyr::mutate(group=case_when(treatment_descr=="Nstemi" ~ "NSTEMI",
                                                      treatment_descr=="Stemi" ~ "STEMI",
                                                      treatment_descr=="UA" ~ "UA",
                                                      treatment_descr=="Healthy" ~ "Healthy")) %>% 
               dplyr::mutate(group== factor(group, levels=c("STEMI", "NSTEMI", "UA", "Healthy"))), 
