...
corr_biomarkers <- cor(table_markers_ACS %>% dplyr::select("ng ccfDNA /mL plasma", "LVEF(%)", "CRP (mg/dl)", "TroponinT (ng/l)", "CK (U/l)", "CK max (U/l)", "CK-MB (U/l)", "CK-MB max (U/l)") %>% dplyr::rename("ccfDNA ng/ml" = "ng ccfDNA /mL plasma"), use = "complete.obs")
```
...
Figure1B <- (corr_biomarkers * upper.tri(corr_biomarkers, diag = TRUE)) %>%
  ...
  scale_size_continuous(labels = c("\u2264 0.1", "\u2264 0.01", "\u2264 0.001", "\u2264 0.0001"), breaks = c(1, 2, 3, 4)) +
...
Figure_1 <- ggarrange(Figure1A, Figure1B, ncol = 2, widths = c(1, 2), align = "h", labels = c("A", "B"))
ggsave(
  bg = "white",
  "/local/AAkalin_cardiac/Results/cardiac/Plots/Figure1.pdf",
  device = "pdf",
  plot = Figure_1, width = 210, units = "mm", height = 110
)
Figure_1
```
...
Figure3B <- ggarrange(Figure3B, labels = "B")
...
pdf(
  file = "/local/AAkalin_cardiac/Results/cardiac/Plots/Figure3A.pdf",
  bg = "white"
)
Figure_3A
grid.text("A", x = 0.03, y = .97, gp = gpar(fontsize = 21, fontface = "bold"))
dev.off()
```
...
```{r}
corr_biomarkers <- cor(DMR_percMeth_ACS_markers %>% dplyr::select("treatment", "ccfDNA_ng_per_ml_plasma", "LVEF(%)", "CRP (mg/dl)", "TroponinT (ng/l)", "CK (U/l)", "CK max (U/l)", "CK-MB (U/l)", "CK-MB max (U/l)"), use = "complete.obs", method = "pearson")
```
...
```{r Publication_Figure_5C,fig.width=20,fig.height=4}
...
counts <- as_tibble(counts) %>%
  ...
  dplyr::mutate(marker = case_when(
    marker == "LVEF(%)" ~ "LVEF [%]",
    marker == "CRP (mg/dl)" ~ "CRP [mg/dl]",
    marker == "TroponinT (ng/l)" ~ "TroponinT [ng/l]",
    marker == "CK (U/l)" ~ "CK [U/l]",
    marker == "CK max (U/l)" ~ "CK max [U/l]",
    marker == "CK-MB (U/l)" ~ "CK-MB [U/l]",
    marker == "CK-MB max (U/l)" ~ "CK-MB max [U/l]",
    TRUE ~ "Disease"
  ))
...
```{r Publication_Figure_5,}
Figure_5AB <- ggarrange(Figure_5A, Figure_5B, ncol = 2, nrow = 1, labels = c("A", "B"), align = "h")
Figure_5C <- ggarrange(Figure_5C, labels = c("C"), font.label = list(size = 30))

ggsave(
  bg = "white",
  "/local/AAkalin_cardiac/Results/cardiac/Plots/Figure5AB.pdf",
  device = "pdf",
