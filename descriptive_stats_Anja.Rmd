...
tbl1<-TBL %>% filter(treatment_descr!="CAD+posStressEcho+pos" & treatment_descr!="CAD+posStressEcho+neg" ) %>% rename("Sample"="sample_ids")
```
Load meta data
...
names(suppl_tbl_2) <- c("Sample","treatment_descr","treatment" ,  "BS_conversion_rate" ,"Average_CpG_coverage" , "CpGs coverage>=3",
...
sample_info<-sample_info %>% rename("total_amount_cfDNA_ug"="total amount (ng/50uL)","Sample"="code for Novogen")
sample_info2 <- read_delim("/home/arathge/from-rafael/sample_info2.csv", "\t", escape_double = FALSE, trim_ws = TRUE)
...
table_markers<-readxl::read_excel("/home/arathge/from-rafael/WGBS_ShortTableCorrelations.xlsx")
table_markers<-merge(suppl_tbl_2,table_markers,by="Sample")
table_markers = table_markers %>% rename("CRP (mg/dl)"="CRP(mg/dl_<5)","TroponinT (ng/l)"="TroponinThs_t1 (ng/l <14or<50)","CK (U/l)"="CK (U/l _<190)","CK max (U/l)"="CKmax(U/l_<190)","CK-MB (U/l)"="CK-MB(U/l <24)","CK-MB max (U/l)"="CK-MB_Max")
...
#write.table(table_markers %>% select("Sample", "LVEF(%)","CRP (mg/dl)","TroponinT (ng/l)","CK-MB max (U/l)") ,file = "~/clinical.input.csv",sep = "\t")
```
...
corr_biomarkers <- cor(table_markers_ACS  %>% dplyr::select("ng ccfDNA /mL plasma","LVEF(%)","CRP (mg/dl)","TroponinT (ng/l)", "CK (U/l)" , "CK max (U/l)" ,"CK-MB (U/l)","CK-MB max (U/l)") %>% rename("ccfDNA ng/ml"="ng ccfDNA /mL plasma"), use = "complete.obs")
```

```{r Save_Correlation_Plot}

# pdf(file = "/local/AAkalin_cardiac/Results/cardiac/Plots/Figure1b.pdf")
# corrplot(corr_biomarkers, tl.col = "brown", tl.srt = 30, bg = "White",
#          title = "",
#          type = "lower")
# dev.off()
```
...
```{r Export_Figure_1, fig.height = 16, fig.width = 12}
# pdf(file = "/local/AAkalin_cardiac/Results/cardiac/Plots/FigureS1.pdf")
# ggarrange(plot_a, plot_b, plot_c,  plot_d,plot_e,plot_f, 
...
p1<-ggplot(suppl_tbl_2, aes(x=Sample, y=BS_conversion_rate, fill=treatment_descr)) + 
...
p2<-ggplot(suppl_tbl_2, aes(x=Sample, y=Average_CpG_coverage, fill=treatment_descr)) + 
  geom_bar(stat = "identity") + scale_y_continuous(expand = expansion(mult = c(0, .1))) + 
  scale_fill_Publication() +theme_Publication() +
  theme(axis.text.x = element_text(angle = 90),axis.title.x = element_blank()) + 
  theme(legend.title = element_blank()) + 
...
```{r Export_Figure_2, fig.height = 8, fig.width = 20}
# pdf(file = "/local/AAkalin_cardiac/Results/cardiac/Plots/FigureS2.pdf",height = 8,width = 20)
# cowplot::plot_grid(p1,p2,labels="AUTO",rel_widths  = c(19/40, 22/40))
# dev.off()
```
...
```{r Amout_cfDNA_Plot,fig.height = 5, fig.width = 8}
library(gridGraphics)
ggplot(suppl_tbl_2, aes(x=as.factor(treatment_descr), y=cfDNA_ug_per_ml_plasma, fill=as.factor(treatment_descr))) + 
      geom_boxplot(alpha=0.9) + 
      geom_jitter(shape=16, position=position_jitter(0.2)) + 
      xlab("") +
      ylab("Total cfDNA (ng) / mL plasma") +
...
# ggsave(
#   "/local/AAkalin_cardiac/Results/cardiac/Plots/Figure1A.pdf",
#   device = "pdf",
#   plot = last_plot())
```

```{r}
x <-suppl_tbl_2 %>% filter(treatment_descr  =="Healthy") %>% dplyr::select(total_amount_cfDNA_ug)
y <-suppl_tbl_2 %>% filter(treatment_descr  =="ACS_Stemi") %>% dplyr::select(total_amount_cfDNA_ug)
wilcox.test(x$total_amount_cfDNA_ug,y$total_amount_cfDNA_ug, alternative = "two.sided")

y <-suppl_tbl_2 %>% filter(treatment_descr  =="ACS_Nstemi") %>% dplyr::select(total_amount_cfDNA_ug)
wilcox.test(x$total_amount_cfDNA_ug,y$total_amount_cfDNA_ug, alternative = "two.sided")

y <-suppl_tbl_2 %>% filter(treatment_descr  =="ACS_UA") %>% dplyr::select(total_amount_cfDNA_ug)
wilcox.test(x$total_amount_cfDNA_ug,y$total_amount_cfDNA_ug, alternative = "two.sided")


x <-suppl_tbl_2 %>% filter(treatment_descr  =="ACS_UA") %>% dplyr::select(total_amount_cfDNA_ug)
y <-suppl_tbl_2 %>% filter(treatment_descr  =="ACS_Nstemi") %>% dplyr::select(total_amount_cfDNA_ug)
wilcox.test(x$total_amount_cfDNA_ug,y$total_amount_cfDNA_ug, alternative = "two.sided")
...
x <-suppl_tbl_2 %>% filter(treatment_descr  =="ACS_Nstemi") %>% dplyr::select(total_amount_cfDNA_ug)
y <-suppl_tbl_2 %>% filter(treatment_descr  =="ACS_Stemi") %>% dplyr::select(total_amount_cfDNA_ug)
wilcox.test(x$total_amount_cfDNA_ug,y$total_amount_cfDNA_ug, alternative = "two.sided")


```
Perform non-parametric two-sided Wicoxon rank sum test between all conditions, but too little observations for all cases
...
meth.deT_ACS=methylKit:::readMethylBaseDB(methT.path,"tabix",
                                      ...
                                      "region",treatment_acs,TRUE,skip=0)
```
...
annot_ord_genomics<- c("hg38_genes_1to5kb", "hg38_genes_3UTRs", "hg38_genes_5UTRs" , "hg38_genes_exons" , 
...
ggplot(all_annot_summary_alltypes, aes(x=annot.type, y=n,fill=group))+
  ...
  scale_fill_manual(values=group.colors_3) + scale_fill_Publication() +theme_Publication()
  
# ggsave(
#   "/local/AAkalin_cardiac/Results/cardiac/Plots/Figure3B.pdf",
#   device = "pdf",
#  plot = last_plot())
# dev.off()
```
...
Digenet_enrch_for_heat <-  Digenet_enrch %>% group_by(ID,Description) %>% summarise(qvalue,group) %>%  filter(any(qvalue < 0.05)) %>% filter(str_detect(Description, regex(terms_for_regex, ignore_case = T)))
...
rgreat_enrch_for_heat <-  rgreat_enrch %>% group_by(ID,name) %>% summarise(Binom_Adjp_BH,group) %>%  filter(any(Binom_Adjp_BH < 0.05))
...
```{r}
f4a<-ggplot(rgreat_enrch_for_heat_na , aes(x = group, y = name, fill = -log10(Binom_Adjp_BH) )) +
  labs(fill = "-log10(qvalue)") +
  ylab("Pathway") +
  xlab("") +
  geom_tile() + 
  geom_text(aes(label=annot)) +
  theme_Publication() + scale_fill_gradient(low = "white", high = "red") 

f4b<-ggplot(Digenet_enrch_for_heat_na , aes(x = group, y = Description, fill = -log10(qvalue) )) +
  ylab("Disease") +
  xlab("") +
  geom_tile() +
  geom_text(aes(label=annot)) +
...
# cowplot::plot_grid(f4a,f4b,labels="AUTO",rel_widths  = c(2/4, 2/4))
# dev.off()
```
...
all_g <- all_g %>% group_by(seqnames,start,end) %>% summarize_all(~paste(unique(.), collapse = ','))
##
...
# pdf(file="/local/AAkalin_cardiac/Results/cardiac/Plots/Figure3A.pdf",width=8, height=6)
# UpSetR::upset(UpSetR::fromList(v_ACS),sets=c("UA","Nstemi","Stemi"),keep.order=TRUE,text.scale=1.8)
# dev.off()
...
for ( n in marker_names){
  ...
  test <- lm(DMR_percMeth_ACS_markers[[n]] ~ DMR_percMeth_ACS_markers$ccfDNA_ng_per_ml_plasma)
  ...
  for ( i in dmr_names ){
    #tmp <- DMR_percMeth_ACS_markers %>% select( all_of(n), all_of(i),ccfDNA_ng_per_ml_plasma )
    
    #LM biomarkers ~ methylation DMRs + cfDNA
    test2<-lm(DMR_percMeth_ACS_markers[[n]] ~ DMR_percMeth_ACS_markers[[i]] + scale(DMR_percMeth_ACS_markers$ccfDNA_ng_per_ml_plasma ))
  ...
  test_all_adj=test_all_adj[!grepl('ccfDNA_ng_per_ml_plasma', row.names(test_all_adj)),]
  test_all_adj=rownames_to_column(as.data.frame(test_all_adj), var = "DMR")
  ...
  test_all_3=rownames_to_column(as.data.frame(test_all_3), var = "DMR")
  ...
  test_anova_all =test_anova_all %>% filter(`Pr(>F)` <= 0.05)
...
```{r}
corr_biomarkers <- cor(DMR_percMeth_ACS_markers  %>% dplyr::select("treatment","ccfDNA_ng_per_ml_plasma","LVEF(%)","CRP(mg/dl_<5)","TroponinThs_t1 (ng/l <14or<50)", "CK (U/l _<190)" , "CKmax(U/l_<190)" ,"CK-MB(U/l <24)","CK-MB_Max"), use = "complete.obs",method = "pearson")
```
...
pca_out<-merge(as.data.frame(pca_df),suppl_tbl_2 %>% select(Sample,treatment_descr) %>% remove_rownames %>% column_to_rownames(var="Sample"),by=0)
...
p<-ggplot(pca_out,aes(x=PC1,y=PC2,color=group,label=Row.names,fill=as.factor(treatment_descr) )) + 
...
# ggsave(
#   "/local/AAkalin_cardiac/Results/cardiac/Plots/Figure5b.pdf",
#   device = "pdf",
#   plot = last_plot())  
```
...
percMeth_ACS_DMRs_sig_lm_for_heat=merge(as.data.frame((percMeth_ACS_DMRs_sig_lm_for_heat  %>% rename("Sample"="sample"))) ,suppl_tbl_2 %>% select(Sample,treatment_descr),by="Sample")

```


```{r,fig.height= 10,fig.width=6}
Heatmap(t(percMeth_ACS_DMRs_sig_lm),clustering_distance_columns = "spearman",column_names_gp = gpar(col = c(rep("red", 8), rep("blue", 8),rep("green", 7),rep("pink", 6))),show_row_names = FALSE)
```
...
perc_Meth_top5_ACS_DMRs_sig_lm <- merge(percMeth_ACS_DMRs_sig_lm %>% select(top_DMR_treatment$DMR),table_markers %>% select(Sample,treatment_descr,`CK max (U/l)`,`LVEF(%)`,`TroponinT (ng/l)`) %>% remove_rownames %>% column_to_rownames(var="Sample"),by=0)
...
perc_Meth_top5_ACS_DMRs_sig_tropo_lm <- merge(percMeth_ACS_DMRs_sig_tropo_lm %>% select(top_DMR_tropo$DMR),table_markers %>% select(Sample,treatment_descr,`CK max (U/l)`,`LVEF(%)`,`TroponinT (ng/l)`) %>% remove_rownames %>% column_to_rownames(var="Sample"),by=0)
...
perc_Meth_top5_ACS_DMRs_sig_CK_max_lm <- merge(percMeth_ACS_DMRs_sig_CK_max_lm %>% select(top_DMR_CK_max_LM$DMR),table_markers %>% select(Sample,treatment_descr,`CK max (U/l)`,`LVEF(%)`,`TroponinT (ng/l)`,`CK-MB max (U/l)`) %>% remove_rownames %>% column_to_rownames(var="Sample"),by=0)
...
perc_Meth_top5_ACS_DMRs_sig_CK_MB_max_lm <- merge(percMeth_ACS_DMRs_sig_CK_MB_max_lm %>% select(top_DMR_CK_MB_max_LM$DMR),table_markers %>% select(Sample,treatment_descr,`CK max (U/l)`,`LVEF(%)`,`TroponinT (ng/l)`,`CK-MB max (U/l)`) %>% remove_rownames %>% column_to_rownames(var="Sample"),by=0)
...
ggscatter(perc_Meth_top5_ACS_DMRs_sig_tropo_lm %>% filter(-treatment_descr !=0), y = "value", x = "TroponinT (ng/l)", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "TroponinT (ng/l)",ylab = "% Methylation")+ facet_wrap(~variable,ncol = 5) + 
...
```{r,fig.width=20,fig.height=4}
ggscatter(perc_Meth_top5_ACS_DMRs_sig_lm %>% filter(-treatment_descr !=0), y = "value", x = "treatment_descr", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Disease severity: 1 - UA, 2- Nstemi, 3 - Stemi", ylab = "% Methylation" ,xlim=c(1,3) )+ facet_wrap(~variable,ncol = 5) + 
scale_x_continuous(breaks=c(1,2,3)) + theme(text = element_text(size=18))
# ggsave(
...
ggscatter(perc_Meth_top5_ACS_DMRs_sig_CK_max_lm %>% filter(-treatment_descr !=0), y = "value", x = "CK max (U/l)", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "CK max (U/l)", ylab = "% Methylation" )+ facet_wrap(~variable,ncol = 5)
```
...
```{r}
ggscatter(perc_Meth_top5_ACS_DMRs_sig_lm %>% filter(-treatment_descr !=0), y = "treatment_descr", x = "CKmax(U/l_<190)", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          ylab = "Disease severity", xlab = "CKmax(U/l_<190)")
```
...
```{r Export_Figure_5a,fig.height= 9,fig.width=9}
p<-ggplot(counts, aes(x=marker, y=Percentage)) +
  geom_bar(stat="identity") +theme_Publication() + theme(axis.text.x = element_text(angle = 0),axis.title.y = element_blank()) +
  coord_flip() + theme(text = element_text(size=20))
p

ggsave(
  "/local/AAkalin_cardiac/Results/cardiac/Plots/Figure5a.pdf",
  device = "pdf",
  plot = last_plot())  
```
